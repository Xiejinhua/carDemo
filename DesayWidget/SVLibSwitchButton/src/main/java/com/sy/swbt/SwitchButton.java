...
public class SwitchButton extends CompoundButton {
    ...
    public static final float DEFAULT_THUMB_RANGE_RATIO = 1.85f;   //默认总体宽度与thumb的比例,t1j
    public static final int DEFAULT_THUMB_SIZE_DP = 20;
    public static final float DEFAULT_THUMB_SIZE_RADIUS_MINUS_DP = 0f;   //thumb半径在缩小时需要减去的大小
    public static final float DEFAULT_THUMB_MARGIN_DP = 2.0f;   //thumb外边距
    public static final int DEFAULT_ANIMATION_DURATION = 250;   //默认动画时常
    public static final int DEFAULT_TINT_COLOR = 0x327FC2;
    public static final float DEFAULT_DISABLED_ALPHA = 1f;    //默认不可用透明度
    private static final float DEFAULT_TEXT_SIZE = 16;  //默认thumb字体大小
    ...
    private RectF mThumbRectF, mBackRectF, mSafeRectF, mTextOnRectF, mTextOffRectF, mBtTextRectF;
    ...
    private String mBtTextOn;   //thumb上“开”提示文字
    private String mBtTextOff;  //thumb上“关”提示文字
    private int mBtTextColor;   //thumb上提示文字颜色
    private float mBtTextSize;  //thumb上提示文字大小
    ...
    private void init(AttributeSet attrs) {
        drawableIds = new int[]{R.mipmap.img_btn_0, R.mipmap.img_btn_1, R.mipmap.img_btn_2, R.mipmap.img_btn_3,
                R.mipmap.img_btn_4, R.mipmap.img_btn_5, R.mipmap.img_btn_6, R.mipmap.img_btn_7,
                R.mipmap.img_btn_8, R.mipmap.img_btn_9, R.mipmap.img_btn_10};
        mTouchSlop = ViewConfiguration.get(getContext()).getScaledTouchSlop();
        mClickTimeout = ViewConfiguration.getPressedStateDuration() + ViewConfiguration.getTapTimeout();//长按超时时间，不使用
        ...
        float disabledAlpha = DEFAULT_DISABLED_ALPHA;   //默认不可见透明图
        boolean fadeBack = false;   //默认，是否有透明度变化效果
        int tintColor = 0;
        String textOn = null;
        String textOff = null;
        String btTextOn = null;
        String btTextOff = null;
        float btTextSize = DEFAULT_TEXT_SIZE;  //默认，thumb上提示文字大小
        int btTextColor = Color.BLACK;  //默认，thumb上提示文字颜色
        ...
        if (ta != null) {
            disabledAlpha = ta.getFloat(R.styleable.SwitchButton_kswDisabledAlpha, disabledAlpha);
            ...
            //获取thumb宽度，提供默认
            thumbWidth = ta.getDimension(R.styleable.SwitchButton_kswThumbWidth, res.getDimension(R.dimen.ksw_md_switch_default_height));
            //获取thumb高度，提供默认
            thumbHeight = ta.getDimension(R.styleable.SwitchButton_kswThumbHeight, res.getDimension(R.dimen.ksw_md_switch_default_height));
            ...
            fadeBack = ta.getBoolean(R.styleable.SwitchButton_kswFadeBack, fadeBack);
            ...
            btTextOn = ta.getString(R.styleable.SwitchButton_kswBtTextOn);
            btTextOff = ta.getString(R.styleable.SwitchButton_kswBtTextOff);
            btTextSize = ta.getDimension(R.styleable.SwitchButton_kswBtTextSize, btTextSize);
            btTextColor = ta.getColor(R.styleable.SwitchButton_kswBtTextColor, btTextColor);
        ...
        if (mTintColor == 0) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                found = getContext().getTheme().resolveAttribute(android.R.attr.colorAccent, typedValue, true);
            }
            if (found) {
...
//            mCurrThumbColor = mThumbColor.getDefaultColor();
            //默认thumb颜色
            mThumbColor = res.getColorStateList(R.color.ksw_md_thumb_color, null);
        }
...
//            mNextBackColor = mBackColor.getColorForState(CHECKED_PRESSED_STATE, mCurrBackColor);
            //默认背景颜色
            mBackColor = res.getColorStateList(R.color.ksw_md_back_color, null);
        }
        ...
        // thumbSize
        mThumbWidth = ceil(thumbWidth - marginLeft - marginRight);  //减去外边距，以免影响控件大小
        mThumbHeight = ceil(thumbHeight - marginTop - marginBottom);
        ...
        setAlpha(isEnabled() ? 1f : mDisabledAlpha);
    ...
    boolean isTimeAlivable() {
        long tt = System.currentTimeMillis() - mCurrTime;
        if (tt > mMaxTime) {
...
//                moveRange = ceil(mThumbWidth * mThumbRangeRatio);
                moveRange = ceil((mThumbWidth + (mThumbMargin.left + mThumbMargin.right) / 2) * mThumbRangeRatio);    /** 将减去的外边距加入计算，保证比例正确*/
...
//            moveRange = ceil(mThumbWidth * mThumbRangeRatio);
        ...
        //当thumb水平外边距<0，向外伸展时，将整体宽度修正，防止比例错误
        if (mThumbMargin.left < 0 || mThumbMargin.right < 0) {
            measuredWidth = (int) (measuredWidth + Math.abs(mThumbMargin.left + mThumbMargin.right));
        }
    ...
    private void setup() {
        ...
        if (mThumbRadius == -1) {
            mThumbRadius = (float) Math.min(mThumbWidth, mThumbHeight) / 2;
        }
        if (mBackRadius == -1) {
            mBackRadius = (float) Math.min(mBackWidth, mBackHeight) / 2;
        }
        ...
        if (contentHeight <= drawingHeight) {
            ...
            thumbTop = getPaddingTop() + Math.max(0, mThumbMargin.top) + (float) (contentHeight - drawingHeight + 1) / 2;
        }
        ...
        if (contentWidth <= mBackWidth) {
            ...
            thumbLeft = getPaddingLeft() + Math.max(0, mThumbMargin.left) + (float) (contentWidth - drawingWidth + 1) / 2;
        }
        ...
        //当thumb水平外边距<0，向外伸展时，将背景track右边修正
        if (mThumbMargin.left < 0 || mThumbMargin.right < 0) {
            mBackRectF.set(backLeft,
                    mThumbRectF.top - mThumbMargin.top,
                    backLeft + mBackWidth + mThumbMargin.left + mThumbMargin.right,
...
//        Bitmap bitmap = ((BitmapDrawable)getResources().getDrawable(drawableIds[(int) (getProgress()*10)])).getBitmap();
//        canvas.drawBitmap(bitmap, 0, 0, null);
        ...
        if (mIsThumbUseDrawable) {  //使用了Drawable作为thumb
            ...
            //缩放动效处理
            float temp = (1 - mProgress) * DEFAULT_THUMB_SIZE_RADIUS_MINUS_DP;  //缩放减去部分
            mPresentThumbRectF.set(mPresentThumbRectF.left + temp, mPresentThumbRectF.top + temp, mPresentThumbRectF.right - temp, mPresentThumbRectF.bottom - temp);

            canvas.drawRoundRect(mPresentThumbRectF, mThumbRadius, mThumbRadius, mPaint);
        }
        //thumb text
        String btText = getProgress() > 0.5 ? mBtTextOn : mBtTextOff;
        mPaint.setColor(mBtTextColor);
        mPaint.setTextSize(mBtTextSize);
        ...
        if (mBtTextOn != null) {
            canvas.drawText(btText, mPresentThumbRectF.centerX(), baseLineY, mPaint);
        }
...
//                if (Math.abs(deltaX) < mTouchSlop && Math.abs(deltaY) < mTouchSlop && time < mClickTimeout) {
                if (Math.abs(deltaX) < mTouchSlop && Math.abs(deltaY) < mTouchSlop && isTimeAlivable()) {   //不计超时
                    if (mEnableClickThumb || !mPresentThumbRectF.contains(mStartX, mStartY))
                        performClick();
                } else {
                    boolean nextStatus = getStatusBasedOnPos();
                    //拖动滑块但点击时间过短时，应当认为是做了点击事件//
                    upTime = System.currentTimeMillis();
                    if(upTime - pressTime < 300){
//                        Log.d(TAG, "onTouchEvent: 应该直接跳转下一状态");
    ...
    public void setChecked(final boolean checked) {
        //初始化进入界面时不显示动画效果
        if (getWindowToken() != null && mIsLaidOut) {  //已初始化完成
            ...
            setCheckedImmediately(checked); //界面未初始化则直接展示选中状态，无动画
        }
    }

    //部分情况可能isLaidOut为true后又变为false，使用变量记录true后一直返回true，防止动画不显示
    private boolean mIsLaidOut = false;
    private boolean mIsFirstLaidOut = true;  //是否第一次经历isLaidOut()
    @Override
    public boolean isLaidOut() {
        if(mIsFirstLaidOut){
            mIsFirstLaidOut = false;
            mIsLaidOut = true;
        }
    ...
    public void setThumbDrawableRes(int thumbDrawableRes) {
        setThumbDrawable(getContext().getResources().getDrawable(thumbDrawableRes));
    }
    ...
    public void setBackDrawableRes(int backDrawableRes) {
        setBackDrawable(getContext().getResources().getDrawable(backDrawableRes));
    }
    ...
    public void setBackColorRes(int backColorRes) {
        setBackColor(getContext().getResources().getColorStateList(backColorRes));
    }
    ...
    public void setThumbColorRes(int thumbColorRes) {
        setThumbColor(getContext().getResources().getColorStateList(thumbColorRes));
    }
    ...
    public void setEnabled(boolean enabled) {
        ...
        setAlpha(enabled ? 1f : mDisabledAlpha);
    }
}