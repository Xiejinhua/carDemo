...
import com.desaysv.psmap.databinding.ViewSearchBoxBinding
...
class SearchBoxView @JvmOverloads constructor(
    ...
    private var binding: ViewSearchBoxBinding

    init {
        binding = ViewSearchBoxBinding.inflate(LayoutInflater.from(context), this, true)
    ...
    //搜索框长度
    fun searchBoxLength(length: Int) {
        Timber.i("searchBoxLength length:$length")
        val params = binding.searchBox.layoutParams as LayoutParams
        params.width = length
        binding.searchBox.layoutParams = params

        val paramsContent = binding.searchContent.layoutParams as LayoutParams
        paramsContent.width = length - binding.searchContent.resources.getDimensionPixelSize(com.desaysv.psmap.base.R.dimen.sv_dimen_10)
        binding.searchContent.layoutParams = paramsContent
    }

    //返回键是否显示
    fun showBackBtn(show: Boolean = true) {
        binding.searchBack.visibility = if (show) VISIBLE else GONE
    }
    ...
    //离线城市按钮点击操作事件
    fun switchCityOnClickListener(listener: OnClickListener?) {
        binding.clOfflineCity.setOnClickListener(listener)
        ViewClickEffectUtils.addClickScale(binding.clOfflineCity, CLICKED_SCALE_95)
    }

    fun showCityBtn(show: Boolean) {
        binding.clOfflineCity.visibility = if (show) VISIBLE else GONE
    }

    //设置离线搜索城市名称
    fun setCityName(title: String?) {
        binding.tvOfflineCityTitle.text = title
    }
