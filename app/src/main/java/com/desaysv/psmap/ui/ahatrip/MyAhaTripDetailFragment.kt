...
import com.desaysv.psmap.databinding.FragmentMyAhaTripDetailBinding
...
import com.desaysv.psmap.ui.adapter.MyAhaLineDetailDayAdapter
 ...
 */
@AndroidEntryPoint
class MyAhaTripDetailFragment : Fragment() {
    private lateinit var binding: FragmentMyAhaTripDetailBinding
    private val viewModel: MyAhaTripDetailViewModel by viewModels()
    ...
    private var ahaLineDetailDayAdapter: MyAhaLineDetailDayAdapter? = null
    ...
    override fun onResume() {
        ...
        if (viewModel.selectTab.value == 0) {
            viewModel.setGuidePointLine() //简介图层扎标和画线
        } else if (viewModel.selectTab.value!! > 0) {
            viewModel.setGuideDayNodePointLine(viewModel.selectTab.value!!) //DAY几图层扎标和画线
        }
    }
    ...
    override fun onDestroy() {
        ...
        viewModel.guideDetail.postValue(null)
    }
    ...
    override fun onCreateView(
        ...
        binding = FragmentMyAhaTripDetailBinding.inflate(inflater, container, false)
        return binding.root
    }
    ...
    private fun initBinding() {
        ...
        viewModel.setGuideId(arguments?.getInt("id", 0).toString())
        ...
        if (viewModel.guideDetail.value == null) {
            viewModel.requestMineGuideDetail() //共创路书详情
        } else {
            setRadioGroupLayout(viewModel.guideDetail.value) //设置RadioGroup布局，实现简介，dayXX的切换
            ...
            if (viewModel.guideDetail.value != null) {
                setCarousel(viewModel.guideDetail.value!!.images) //设置轮播图
            }
        }
        binding.banner.addBannerLifecycleObserver(this)//添加生命周期观察者
            .setIndicator(CircleIndicator(requireContext()), false)
        ahaLineDetailDayAdapter = MyAhaLineDetailDayAdapter().also { binding.nodeList.adapter = it }
    ...
    private fun initEventOperation() {
        ...
        binding.sound.setDebouncedOnClickListener {
            viewModel.guideDetail.value?.description?.let {
        ...
        binding.favorite.setDebouncedOnClickListener {
            ...
            if (viewModel.isLogin()) {
                viewModel.requestFavorite(viewModel.guideDetail.value?.id.toString(), isMineFav) //路书请求收藏/取消收藏
            } else {
        ...
        binding.allDayNavi.setDebouncedOnClickListener {
            try {
                ...
                val lineDetail = viewModel.guideDetail.value?.nodeList?.get(selectTab - 1)?.node ?: return@setDebouncedOnClickListener
        ...
        ahaLineDetailDayAdapter?.setOnItemClickListener { _, _, position ->
            val guideNode = ahaLineDetailDayAdapter?.data?.get(position)
            Timber.i("setOnItemClickListener id:${guideNode?.nodeId}")
            if (guideNode != null) {
                mSpeechSynthesizeBusiness.nowStopPlay() //立即停止播报
                findNavController().navigate(R.id.to_ahaScenicDetailFragment, Bundle().apply {
                    putInt("id", guideNode.nodeId)
                })
            } else {
        ...
        viewModel.guideDetail.unPeek().observe(viewLifecycleOwner) {
            if (it != null) {
                setRadioGroupLayout(it) //设置RadioGroup布局，实现简介，dayXX的切换
                viewModel.setDescriptionData(it)
                setCarousel(it.images) //设置轮播图
                viewModel.setGuidePointLine() //简介图层扎标和画线
            }
        }
        ...
        viewModel.selectTab.observe(viewLifecycleOwner) {
            if (it > 0) {
                val node = viewModel.guideDetail.value?.nodeList
                if (!node.isNullOrEmpty()) {
                    ahaLineDetailDayAdapter?.onRefreshData(viewModel.guideDetail.value?.nodeList?.get(it - 1)?.node)
                }
                viewModel.hasNode.postValue(!node.isNullOrEmpty())
                binding.banner.stop()
                viewModel.setGuideDayNodePointLine(it) //DAY几图层扎标和画线
            } else {
        ...
        viewModel.gotoAhaDayDetail.unPeek().observe(viewLifecycleOwner) {
            ...
            val node = viewModel.guideDetail.value?.nodeList
            if (!node.isNullOrEmpty()) {
                ahaLineDetailDayAdapter?.onRefreshData(viewModel.guideDetail.value?.nodeList?.get(it)?.node)
            }
            viewModel.hasNode.postValue(!node.isNullOrEmpty())
            binding.banner.stop()
            viewModel.setGuideDayNodePointLine(it + 1) //DAY几图层扎标和画线
        }

        viewModel.isGuideFavChange.unPeek().observe(viewLifecycleOwner) {
            try {
                if (isMineFav) {
                    viewModel.guideCollectList.value?.get(position)?.isFav = it
                    if (!it) {
                        viewModel.deleteGuideCollectResult.postValue(viewModel.guideCollectList.value?.get(position)?.id.toString())
                    } else {
                        viewModel.deleteGuideCollectResult.postValue("")
                    }
                } else {
                    viewModel.guideList.value?.get(position)?.isFav = it
                }
            } catch (e: Exception) {
        ...
        viewModel.mapCommand.unPeek().observe(viewLifecycleOwner) { mapCommand ->
            ...
            mapCommand?.let {
                ...
                when (it.mapCommandType) {
                    MapCommandType.AhaTripCollect -> {
                        val collectMapCommand: Boolean = mapCommand.data.toBoolean()
                        if (collectMapCommand != viewModel.isFav.value) {
                            if (viewModel.isLogin()) {
                                viewModel.requestFavorite(viewModel.guideDetail.value?.id.toString(), isMineFav) //路书请求收藏/取消收藏
                            }
                        }
    ...
    private fun setRadioGroupLayout(data: GuideData?) {
        if (data != null) {
            ...
            if (data.nodeList != null && data.nodeList!!.isNotEmpty()) {
                data.nodeList?.forEachIndexed { index, guideDataDTO ->
                    tabItems.add(AhaTripDetailTabBean(index + 1, index + 1, "第${guideDataDTO.dayId}天"))
                }
            }
            tabItems.forEachIndexed { index, item ->
                val radioButton = SkinRadioButton(requireContext()).apply {
                    ...
                    this.setCompoundDrawablesWithIntrinsicBounds(
                        ...
                        null,
                        null,
                        ContextCompat.getDrawable(requireContext(), R.drawable.selector_bg_rb_transparent)
                    )
    ...
    //设置轮播图
    private fun setCarousel(swiperDTO: List<String>?) {
        ...
        binding.banner.setAdapter(object : BannerImageAdapter<String>(data) {
