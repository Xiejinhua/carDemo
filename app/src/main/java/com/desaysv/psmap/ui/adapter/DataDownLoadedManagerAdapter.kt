...
/**
 * @author 王漫生
 * @project：个性化地图——离线地图DownLoadedAdapter
 */
class DataDownLoadedManagerAdapter(private var mapDataBusiness: MapDataBusiness) :
    BaseDelegateMultiAdapter<DownloadCityDataBean, BaseViewHolder>() {
    ...
    fun setIsUpdate(isUpdate: Boolean = false) {
    ...
    interface OnMapDataItemClickListener {
        fun onStartClick(isProvince: Boolean, item: DownloadCityDataBean, isUpdate: Boolean)
        fun onItemClick(isProvince: Boolean, item: DownloadCityDataBean, isUpdate: Boolean)
    }

    override fun convert(holder: BaseViewHolder, item: DownloadCityDataBean) {
        Timber.i("DataDownLoadedManagerAdapter item:${Gson().toJson(item)}")
        when (holder.itemViewType) {
            TYPE_PROVINCE -> {
                ...
                binding.root.setDebouncedOnClickListener(300L, onClick = {
                    if (onItemClickListener != null && holder.layoutPosition != -1 && holder.layoutPosition < data.size) {
                        Timber.i("DataDownLoadedManagerAdapter TYPE_PROVINCE item点击操作 cityAdcode:${item.cityItemInfo.cityAdcode}")
            ...
            TYPE_CITY -> {
                ...
                binding.workingQueueAdCodeList = arrayListOf()
                binding.executePendingBindings()
                ...
                binding.clItemchildOfflineCity.setDebouncedOnClickListener(300L, onClick = {
                    if (onItemClickListener != null && holder.layoutPosition != -1 && holder.layoutPosition < data.size) {
                        Timber.i("DataDownLoadedManagerAdapter TYPE_CITY item点击操作 cityAdcode:${item.cityItemInfo.cityAdcode}")
                        onItemClickListener!!.onItemClick(
                            false,
                            item,
                            isUpdate
